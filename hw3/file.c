...
void
print_file_dep(const void* val)
{
  const struct file *file = val;
  // if  a target
  if(file->is_target == 1) {
    struct dep* deps = file->deps; //all dependencies of current file
    const char* file_name = file->name;
    while(deps){
      fputs("\t", dp_file_fd);
      fputs(file_name, dp_file_fd);
      fputs(" -> ",dp_file_fd);
      fputs(deps->file->name, dp_file_fd);
      fputs(";\n", dp_file_fd);
      deps = deps->next;
    }
  }
...
void
print_file_dependencies(const char* dp_file)
{
  dp_file_fd = fopen(dp_file, "w");
  if(dp_file_fd == NULL) return;
  fputs("digraph depgraph {\n", dp_file_fd);
  hash_map(&files, print_file_dep);
  fputs("}\n", dp_file_fd);
  fclose(dp_file_fd);
}
